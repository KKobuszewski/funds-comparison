...
from fundsviewer.middlewares import CORRELATION_ID
...
def setup_logging_with_formatters() -> None:
    """Configures logging and include formatters"""
    root = logging.getLogger()
    root.setLevel(logging.DEBUG)
    sys_stdout_handler = logging.StreamHandler(sys.stdout)
    sys_stdout_handler.setLevel(logging.DEBUG)
    sys_stdout_formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")
